...
```{r}
d_grouped <- d %>% 
  group_by(actor.id, actor.age.years, country)

ggplot(d_grouped, aes(x = actor.age.years)) +
  geom_histogram() +
...
dud <- d
dud$decision <- ifelse(d$decision == "accept", 1, 0)
dud <- dud %>% 
  filter(decision == 0 | decision == 1)

d_grouped2 <- dud %>% 
  group_by(country, actor.age.years, trial_type, condition) %>% 
  summarise(mean_decision = mean(decision),
            SE_decision = std.error(decision))
d_grouped2

plot <- ggplot(d_grouped2, aes(x = actor.age.years, y=mean_decision, color = country)) +
         geom_line(position = "dodge", stat = "identity") +
         facet_grid(trial_type ~ condition) +
        theme_few()

ggplot(d_grouped2, aes(x = actor.age.years, y=mean_decision, color = country)) +
         geom_line(position = "dodge", stat = "identity") +
         facet_grid(trial_type ~ condition) +
        theme_few()
```
