...
plot_handlooking_byage_condition <- fvs %>% 
  ...
  xlab("Age (in months)") +
  ylab("Hand Looking") +
  scale_color_discrete(name = "Condition", labels = c("Faces Medium", "Faces Plus"))
...
Resource used for the following code: https://rpubs.com/ndphillips/47112
...
Resource used for the following code: https://www.stat.berkeley.edu/~s133/Random2a.html
...
double_sample_loop_many_ps <- function (p, x) {
  sample_1 <- rnorm(x)
  test_sample_1 <- t.test(sample_1)
  p_value_final <- test_sample_1$p.value
...
x <- 30
p_values_many_ps <- replicate(10000, sapply(list, double_sample_other_2, x=x))
